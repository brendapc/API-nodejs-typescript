...
const rateLimiter = rateLimit({
    ...
    handler(_, res: Response): void {
        res.status(429).send(ApiError.format({code: 429, message: 'Too manu requests to the /forecast endpoint'}))    
    }
})

@Controller('forecast')
@ClassMiddleware(authMiddleware)
export class ForeclastController extends BaseController {
    @Get('')
